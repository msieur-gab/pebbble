...
class MiniPlayer extends HTMLElement {
    ...
    updateProgress() {
        const fill = this.shadowRoot.querySelector('.mini-player__progress-fill');
        if (fill) {
    ...
    render() {
        ...
        // Add event listeners
        const playBtn = this.shadowRoot.querySelector('.mini-player__play-btn');
        playBtn?.addEventListener('click', (e) => this.handlePlayPause(e));

        const miniPlayer = this.shadowRoot.querySelector('.mini-player');
        miniPlayer?.addEventListener('click', () => this.handleExpand());
    }
}
