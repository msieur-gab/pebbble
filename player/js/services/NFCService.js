 ...
 * Extracts the tag serial number (used as decryption key) directly from NFC scan
...
class NFCService {
    ...
    async startReader() {
        ...
        try {
            console.log('NFCService: Creating NDEFReader...');
            ...
            console.log('NFCService: Calling scan()...');
            await this.reader.scan();
            console.log('NFCService: Scan started!');
    ...
    handleReading(event) {
        ...
        // Extract playlistHash from URL
        if (url) {
            playlistHash = this.parsePlaylistHash(url);
        }

        console.log('NFCService: Tag read', { serial: this.formatSerial(serial), url, playlistHash });
